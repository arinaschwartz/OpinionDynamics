...
public final class GenDraw {
    ...
    public void text(double xmin, double xmax, double ymin, double ymax, String s, double targetPercentage) {
        offscreen.setFont(font);
        FontMetrics metrics = offscreen.getFontMetrics();
	double xmid = (xmax+xmin)/2;
        double xs = scaleX(xmid);
	double ymid = (ymin+ymax)/2;
        double ys = scaleY(ymid);
	double z = scaleX(xmax) - scaleX(xmin);
	double targetLen = targetPercentage*z;
    ...
    public String clipStringWidth(String s, double min, double max, double targetPercentage) {
	double targetLen = targetPercentage*(scaleX(max) - scaleX(min));
	return clipString(s, targetLen);
	
    }

    public String clipStringXaxis(String s, double min, double max, double targetPercentage) {
	double targetLen = targetPercentage*(scaleX(max) - scaleX(min));
	return clipString(s, targetLen);
	
    }

    public String clipStringYaxis(String s, double min, double max, double targetPercentage) {
	// scaling flips min and max
	double targetLen = targetPercentage*(scaleY(min) - scaleY(max));
	return clipString(s, targetLen);
	
    }

    public String clipStringHeight(String s, double min, double max, double targetPercentage) {
	// scaling flips min and max
	double targetLen = targetPercentage*(scaleY(min) - scaleY(max));
	return clipString(s, targetLen);
	...
	int end = s.length();
	while ((strWidth(sub) > targetLen) && (end > 1)) {
	    end--;
